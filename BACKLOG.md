...
### Migrate mutations from API routes to Server Actions
- **Context (2026-05-19, `fix/post-mutation-cache-staleness`):** every mutation in this codebase is a POST to an API route from a client component. The Next.js App Router Router Cache (client-side, in-memory) is NOT invalidated by `revalidatePath()` calls from an API route — only by `router.refresh()` on the client OR by `revalidatePath()` called from inside a Server Action (where Next.js wires the invalidation through automatically). Today the codebase compensates manually with a two-layer rule (documented in `lib/cache/revalidate.ts`): server routes call `revalidatePath()`, AND every client handler calls `router.refresh()` after a successful POST. Easy to miss — one forgotten `router.refresh()` and the user sees stale data after a soft navigation (the bug this PR fixed).
- **The fix:** migrate mutation API routes to Server Actions. Server Actions integrate the Router Cache invalidation automatically when `revalidatePath()` is called within them — no client-side `router.refresh()` rule to remember. Affects: `/api/admin/submissions/[id]/{approve,reject,edit}`, `/api/admin/vendors/[id]/{suspend,unsuspend,delete}`, `/api/submissions` + `/api/submissions/[id]/{resubmit,vendor-approve,vendor-request-changes}`, `/api/company-edit`, `/api/product-edit/[appId]`, `/api/onboarding/confirm`, `/api/contact-vendor`. Each becomes an exported async function in a `"use server"` module.
- **Considerations**: Server Action signatures differ (FormData or typed args, not Request/Response); Zod schemas can be reused. Error handling shape changes (return values, not status codes). Rate limiting + honeypot + auth gates carry over. Non-trivial refactor (~14 routes); doable in chunks per domain (admin moderation, vendor submissions, etc.).
- **Trigger:** when the next round of mutation-heavy work surfaces another stale-data bug, OR when the next major Next.js upgrade makes the migration easier, OR as part of a deliberate "tighten the patterns" pass.
