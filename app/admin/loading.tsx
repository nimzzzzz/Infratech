...
/**
 * Loading state for /admin (overview). Mirrors the editorial-admin
 * eyebrow + greeting H1 + 4-stat-card row + recent-activity list.
 */
        ...
        <ul className="mt-10 grid grid-cols-2 gap-px border border-[var(--color-line-strong)] bg-[var(--color-line-strong)] md:grid-cols-4">
          {[0, 1, 2, 3].map((i) => (
            <li key={i} className="bg-[var(--color-surface)] p-5 md:p-6">
              ...
              <div className="mt-2 h-3 w-20 bg-[var(--color-line-strong)]/30" />
            </li>
          ))}
        </ul>
